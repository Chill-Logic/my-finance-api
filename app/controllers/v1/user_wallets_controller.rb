class V1::UserWalletsController < ApplicationController
  ...
  def create
    ...
    return render json: { message: 'Não foi encontrado usuário com esse e-mail.' }, status: :unprocessable_entity if guest.nil?

    wallet = @current_user.owned_wallets.find_by(id: user_wallet_params[:wallet_id])

    return render json: { message: 'Carteira não encontrada.' }, status: :unprocessable_entity if wallet.nil?

    @user_wallet = UserWallet.new(user: guest, wallet: wallet)

    return render json: { message: @user_wallet.errors.full_messages.join(', ') }, status: :unprocessable_entity unless @user_wallet.save

    render json: { message: 'Usuário convidado com sucesso!' }, status: :created
  end

  def accept
    return render json: { message: @user_wallet.errors.full_messages.join(', ') }, status: :unprocessable_entity unless @user_wallet.update(accepted: true)

    render json: { message: 'Convite aceito com sucesso!' }, status: :ok
  end

  def reject
    return render json: { message: @user_wallet.errors.full_messages.join(', ') }, status: :unprocessable_entity unless @user_wallet.discard
  ...
  def set_user_wallet
    ...
    render json: { message: 'Convite não encontrado ou já respondido.' }, status: :unprocessable_entity if @user_wallet.nil?
  end
